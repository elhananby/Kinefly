"""Tracker base classes for motion and intensity tracked bodyparts.

Ported from nodes/bodypart_motion.py and nodes/bodypart_intensity.py.
"""
...
class MotionTrackedBodypart:
    def __init__(self, name=None, params={}, color="white", bEqualizeHist=False):
        ...
        self.color = color
        self.bgra = ui_colors.bgra_dict[color]
        self.bgra_dim = tuple(0.5 * np.array(ui_colors.bgra_dict[color]))
        self.bgra_state = ui_colors.bgra_dict[color]
        self.pixelmax = 255.0
        ...
        self.params = {}
        self.handles = {
            "hinge": ui_handles.Handle(np.array([0, 0]), self.bgra, name="hinge"),
    ...
    def set_params(self, params):
        ...
        # Compute the body-outward-facing angle: from forward body axis to current point.
        if self.name in ["head", "abdomen"]:
            ...
            ptBodyaxis_i = imaging.get_projection_onto_axis(
                self.ptHinge_i, self.ptHingeAbdomen_i, self.ptHingeHead_i
            )
            self.angleBodypart_i = float(
                np.arctan2(
                    self.params["gui"][self.name]["hinge"]["y"] - ptBodyaxis_i[1],
        ...
        self.R = np.array(
            [
                [cosAngleBodypart_i, -sinAngleBodypart_i],
    ...
    def create_mask(self, shape):
        ...
        cv_filled = -1
        cv2.ellipse(
            img, (x, y), (r_outer, r_outer), 0, hi, lo, ui_colors.bgra_dict["white"], cv_filled
        )
        cv2.ellipse(
            img, (x, y), (r_inner, r_inner), 0, 0, 360, ui_colors.bgra_dict["black"], cv_filled
        )
        ...
        if (len(x_list) > 0) and (len(y_list) > 0):
            ...
            logger.warning("%s: Empty mask.", self.name)
            ...
            if self.imgRoiBackground is not None:
                if self.imgRoi is not None:
                    if self.imgRoiBackground.size == self.imgRoi.size:
                        cv2.accumulateWeighted(
                            self.imgRoi.astype(np.float32), self.imgRoiBackground, alphaBackground
                        )
                    else:
                        self.imgRoiBackground = None
                        self.imgRoi = None
                ...
                if (self.imgFullBackground is not None) and (self.roi is not None):
                    self.imgRoiBackground = copy.deepcopy(
                        self.imgFullBackground[self.roi[1] : self.roi[3], self.roi[0] : self.roi[2]]
                    )
                        ...
                        if self.imgRoiBackground.shape == self.imgRoiFg.shape:
                            if bInvertColor:
                                self.imgRoiFg = cv2.absdiff(
                                    self.imgRoiFg, 255 - self.imgRoiBackground.astype(np.uint8)
                                )
                            else:
                                self.imgRoiFg = cv2.absdiff(
                                    self.imgRoiFg, self.imgRoiBackground.astype(np.uint8)
                                )
    ...
    def update_handle_points(self):
        ...
        self.handles["angle_lo"].pt = np.array([x, y]) + np.array(
    ...
    def hit_object(self, ptMouse):
        ...
        if self.params["gui"][self.name]["track"]:
            ...
            if self.handles["hinge"].hit_test(ptMouse):
                tag = "hinge"
    ...
    def draw_handles(self, image):
        ...
        if self.params["gui"][self.name]["track"]:
            for _tagHandle, handle in self.handles.items():
                handle.draw(image)
        else:
            self.handles["hinge"].draw(image)
    ...
    def draw(self, image):
        ...
        if self.params["gui"][self.name]["track"]:
            x = int(self.params["gui"][self.name]["hinge"]["x"])
            y = int(self.params["gui"][self.name]["hinge"]["y"])
            gui_part = self.params["gui"][self.name]
...
class MotionTrackedBodypartPolar(MotionTrackedBodypart):
    def __init__(self, name=None, params={}, color="white", bEqualizeHist=False):
        ...
        self.polartransforms = imaging.PolarTransforms()
        self.create_wfn = imaging.WindowFunctions().create_tukey
    ...
    def create_mask(self, shape):
        ...
        if self.mask.xMin is not None:
            ...
            ptsUnclipped = np.append(
                ptsUnclipped, cv2.ellipse2Poly((x, y), (r_inner, r_inner), 0, hi, lo, delta), 0
            )
            ...
            # Determine how much of the bottom of the polar image to trim off (i.e. rClip).
    ...
    def update_polarimage(self):
        if self.imgRoiFgMasked is not None:
            ...
            gui_part = self.params["gui"][self.name]
            radius_mid = (gui_part["radius_outer"] + gui_part["radius_inner"]) / 2.0
            dr = (gui_part["radius_outer"] - gui_part["radius_inner"]) / 2.0

            self.imgRoiFgMasked[self.imgRoiFgMasked == 255] = 254  # Make room for the +1, next.
            try:
                # +1 so we find cropped pixels (not merely black pixels).
                self.imgRoiFgMaskedPolar = self.polartransforms.transform_polar_elliptical(
                    self.imgRoiFgMasked + 1,
                    self.i_0,
                    self.j_0,
                    raxial=radius_mid,
                    rortho=radius_mid,
            ...
            except imaging.TransformException:
                logger.warning("%s: Mask is outside the image, no points transformed.", self.name)
            ...
            if (
                (self.imgHeadroom is not None)
                and (self.params["gui"][self.name]["subtract_bg"])
                and (self.params[self.name]["saturation_correction"])
            ):
                try:
            ...
            if self.bValidMask:
                if (self.wfnRoiMaskedPolarCropped is None) or (
                    self.imgRoiFgMaskedPolarCropped.shape != self.wfnRoiMaskedPolarCropped.shape
                ):
                    self.wfnRoiMaskedPolarCropped = self.create_wfn(
                        self.imgRoiFgMaskedPolarCropped.shape, self.params[self.name]["feathering"]
                    )

                cropped_f32 = self.imgRoiFgMaskedPolarCropped.astype(np.float32)
                self.imgRoiFgMaskedPolarCroppedWindowed = cv2.multiply(
                    cropped_f32, self.wfnRoiMaskedPolarCropped
                )
...
#
class IntensityTrackedBodypart:
    def __init__(self, name=None, params={}, color="white", bEqualizeHist=False):
        self.name = name
        self.bEqualizeHist = bEqualizeHist
        ...
        self.params = {}
        self.handles = {
            "center": ui_handles.Handle(np.array([0, 0]), self.bgra, name="center"),
            "radius1": ui_handles.Handle(np.array([0, 0]), self.bgra, name="radius1"),
            "radius2": ui_handles.Handle(np.array([0, 0]), self.bgra, name="radius2"),
        }
    ...
    def create_mask(self, shape):
        ...
        bgra = ui_colors.bgra_dict["white"]
    ...
    def update_handle_points(self):
        ...
        pt = np.array([x, y])
        self.handles["center"].pt = pt
        self.handles["radius1"].pt = pt + radius1 * np.array([self.cosAngle, self.sinAngle])
        self.handles["radius2"].pt = pt + radius2 * np.array([self.sinAngle, -self.cosAngle])
