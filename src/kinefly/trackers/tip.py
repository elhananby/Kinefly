"""TipTracker: track the position of the tip of a bodypart (e.g. wingtip).

Ported from nodes/tracker_tip.py. Finds the point at the greatest distance
from the hinge.
"""
...
class TipDetector:
    ...
    def detect(self, image):
        ...
        iSigIntensities = np.where(np.abs(self.intensities) > self.threshold)[0]  # noqa: F841
...
class TipTracker(MotionTrackedBodypartPolar):
    ...
    def get_diagnostics(self) -> dict:
        ...
        return {
            ...
            "angles": [self.iRadius] if self.iRadius is not None else [],
            "gradients": [],
        }
