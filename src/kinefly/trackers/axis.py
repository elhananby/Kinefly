"""AxisTracker: track the body axis.

Ported from nodes/tracker_axis.py.
"""
...
class AxisTracker:
    def __init__(self, name=None, params={}, color="white", bEqualizeHist=False):
        ...
        self.handles = {
            "pt1": ui_handles.Handle(self.pt1_i, self.bgra, name="pt1"),
            "pt2": ui_handles.Handle(self.pt2_i, self.bgra, name="pt2"),
        }
    ...
    def set_params(self, params):
        ...
        self.pt1_i = np.array(
            [
                self.params["gui"][self.name]["pt1"]["x"],
                self.params["gui"][self.name]["pt1"]["y"],
            ],
            dtype=np.int16,
        )
        self.pt2_i = np.array(
            [
                self.params["gui"][self.name]["pt2"]["x"],
                self.params["gui"][self.name]["pt2"]["y"],
            ],
            dtype=np.int16,
        )
    ...
    def get_bodyangle_i(self):
        angle_i = imaging.get_angle_from_points_i(self.pt1_i, self.pt2_i)
    ...
    def draw(self, image):
        if self.params["gui"][self.name]["track"]:
            ...
            cv2.line(
                image,
                (self.pt1_i[0], self.pt1_i[1]),
                (self.pt2_i[0], self.pt2_i[1]),
                self.bgra,
                1,
            )
